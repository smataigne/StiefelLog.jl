...
styles =[:dash, :dot, :dashdot, :dashdotdot,:dash, :dot]
...
margin = 0.3Plots.cm, yaxis=:log,gridlinewidth=1, gridalpha=0.2,minorgrid=true, minorgridalpha=0.02)
count = 1
for i∈eachindex(betas)
    ...
    if β != 0.5
        ...
        plot!(1:Niter, (norms[1:Niter,2].+norms[1:Niter,1])./(norms[1,2]+norms[1,1]), color = colors[count],linewidth=1, label = false)
    end
    norms, Niter = m.logβ(S₁, S₂, β, 0)[3:4]
    plot!(1:Niter, (norms[1:Niter,2].+norms[1:Niter,1])./(norms[1,2]+norms[1,1]), linestyle=styles[i], color = colors[count],linewidth=1, label = L"\beta=" * string(β))
    count +=1
end
