...
for t ∈ 1:100
    for i ∈ 1:N
        ...
        dist[i] = (dist[i] * (t-1) + norm(U₁-U₂)/sqrt(p)/2) / t
...
P= plot(framestyle=:box, legend=(0.6, 0.93),font="Computer Modern", tickfontfamily="Computer Modern",legendfont="Computer Modern",
    ...
    margin = 0.3Plots.cm,gridlinewidth=1, gridalpha=0.2,minorgrid=true, minorgridalpha=0.02)

for  i∈1:Nβ
    plot!(dist, ratios[:,i], label = L"\beta="*string(betas[i]), color=colors[i+1], linewidth=1.5, linestyle= styles[i+1])
end 
